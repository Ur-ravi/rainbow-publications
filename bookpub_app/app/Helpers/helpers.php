...
function redirect(string $url): void {
    ...
    if (strpos($url, 'http://') !== 0 && strpos($url, 'https://') !== 0) {
        ...
        // If BASE_URL points to a different host than the current request,
        // use a relative path instead to avoid cross-domain redirects
        if ($baseHost && $currentHost && $baseHost !== $currentHost) {
            $url = '/' . ltrim($url, '/');
        } else {
