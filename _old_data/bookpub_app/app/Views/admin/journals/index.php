...
if (list) {
    Sortable.create(list, {
        ...
        onEnd: async function() {
            ...
            await adminPost('<?= BASE_URL ?>/admin/journals/reorder', { ids: ids.join(',') });
...
function confirmDelete(url) {
    showConfirm("Are you sure you want to delete this journal? This action cannot be undone.", async () => {
        try {
            ...
            showToast((data && data.message) || 'Deleted successfully', (data && data.success) ? 'success' : 'error');
            if (data && data.success) setTimeout(() => location.reload(), 700);
        } catch (err) {
