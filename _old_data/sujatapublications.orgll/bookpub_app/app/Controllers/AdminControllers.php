...
class AdminJournalController extends Controller {
    ...
    public function __construct() { requireAdmin(); $this->model = new JournalModel(); }

        public function index(): void {
        $journals = $this->model->findAll('', [], 'sort_order ASC');
    ...
    public function edit(string $id): void {
        ...
        if (!$journal) { redirect(BASE_URL . '/admin/journals'); return; }
        $this->adminView('admin/journals/form', compact('journal'));
    }
    ...
    public function store(): void {
        ...
        $this->json(['success' => true, 'message' => 'Journal added!', 'id' => $id, 'item' => $item]);
    }

    public function update(string $id): void {
        $this->csrfCheck();
        $j = $this->model->findById((int)$id);
        if (!$j) $this->json(['success' => false, 'message' => 'Not found.']);
        $data = $this->journalData();
        if (!empty($_FILES['logo']['name'])) {
            $fn = uploadFile($_FILES['logo'], 'journals', ALLOWED_IMAGE_TYPES);
            if ($fn) { if ($j['logo']) deleteFile('journals/' . $j['logo']); $data['logo'] = $fn; }
        }
        $this->model->update($data, 'id = ?', [(int)$id]);
        $this->json(['success' => true, 'message' => 'Journal updated!']);
    }

    public function delete(string $id): void {
        $this->csrfCheck();
        $j = $this->model->findById((int)$id);
        if ($j && $j['logo']) deleteFile('journals/' . $j['logo']);
        $this->model->delete('id = ?', [(int)$id]);
        $this->json(['success' => true, 'message' => 'Journal deleted.']);
    }
