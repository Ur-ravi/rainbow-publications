...
<head>
    ...
    tailwind.config = {
        theme: {
            extend: {
                colors: {
                    primary:   { DEFAULT: '<?= htmlspecialchars(getSetting("primary_color", "#10B981")) ?>', light: '#34D399', dark: '#059669', 50: '#F0FDF4' },
                    secondary: { DEFAULT: '<?= htmlspecialchars(getSetting("secondary_color", "#1F2937")) ?>', light: '#374151', dark: '#111827' },
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { font-family: 'Inter', sans-serif; color: var(--text-primary); background: var(--slate-50); }

        /* Typography System */
        .heading-xl { font-size: 3.5rem; line-height: 1.1; font-weight: 800; letter-spacing: -0.02em; }
        .heading-lg { font-size: 2.5rem; line-height: 1.2; font-weight: 700; letter-spacing: -0.01em; }
        .heading-md { font-size: 1.875rem; line-height: 1.3; font-weight: 700; }
        .heading-sm { font-size: 1.25rem; line-height: 1.4; font-weight: 600; }
        .label { font-size: 0.75rem; letter-spacing: 0.1em; text-transform: uppercase; font-weight: 600; color: var(--slate-600); }
        ...
        .link-arrow i { font-size: 0.75rem; transition: transform 0.3s ease; }
    ...
    background-size: cover;
        ...
        /* Stat Counter */
        .counter-num { font-weight: 800; font-family: 'IBM Plex Mono', monospace; }

        /* News/Blog Grid */
        .news-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 2rem; }

        /* Board Members Grid */
        .board-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 2rem; }
        ...
        .cta-section .heading-lg { color: white; margin-bottom: 1rem; }
        ...
        .empty-state i { font-size: 3rem; color: var(--slate-300); margin-bottom: 1rem; display: block; }
        ...
        @media (max-width: 768px) {
            ...
            .board-grid { grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); }
        }
        ...
        /* ══════════════════════════════════════════════
           ...
           ══════════════════════════════════════════════ */
        .card, .book-card, .service-card, .journal-card,
        .testimonial-card, .board-member, .conference-card {
            position: relative;
            overflow: hidden;
            will-change: transform;
        }
        .card::before, .book-card::before, .service-card::before, .journal-card::before,
        .testimonial-card::before, .board-member::before, .conference-card::before {
            content: '';
            position: absolute;
            inset: 0;
            z-index: -1;
        ...
        .card:hover::before, .book-card:hover::before, .service-card:hover::before, .journal-card:hover::before,
        .testimonial-card:hover::before, .board-member:hover::before, .conference-card:hover::before {
            opacity: 1;
        }
        ...
        .hero-stats .bg-white\/10 { will-change: transform, opacity; }
    </style>
...
$cards = [
    [
        "title" => "The Midnight Library", "author" => "Matt Haig", "genre" => "Fiction", "year" => "2024",
        "img" => "https://images.unsplash.com/photo-1663735498345-39c452a6d5d8?w=260&h=360&fit=crop&auto=format",
        "bg" => "#0d1225", "accent" => "#003098",
        "pos" => "top:8%; left:5%;", "rotate" => -6, "parallax" => 0.055, "delay" => "0s",
    ],
    [
        "title" => "Sapiens", "author" => "Yuval Noah Harari", "genre" => "Non-Fiction", "year" => "2024",
        "img" => "https://images.unsplash.com/photo-1529589941132-43606325dfb4?w=260&h=360&fit=crop&auto=format",
        "bg" => "#0d2218", "accent" => "#0067b2",
        "pos" => "top:5%; right:5%;", "rotate" => 7, "parallax" => -0.065, "delay" => "-3s",
    ],
    [
        "title" => "Invisible Cities", "author" => "Italo Calvino", "genre" => "Literary", "year" => "2023",
        "img" => "https://images.unsplash.com/photo-1502979932800-33d311b7ce56?w=260&h=360&fit=crop&auto=format",
        "bg" => "#0b1820", "accent" => "#0067b2",
        "pos" => "bottom:10%; left:5%;", "rotate" => 5, "parallax" => 0.04, "delay" => "-5s",
    ],
    [
        "title" => "One Hundred Years", "author" => "G. García Márquez", "genre" => "Classic", "year" => "2023",
        "img" => "https://images.unsplash.com/photo-1648563643923-2091f9c0c12f?w=260&h=360&fit=crop&auto=format",
        "bg" => "#1a0f22", "accent" => "#0067b2",
        "pos" => "bottom:8%; right:5%;", "rotate" => -5, "parallax" => -0.05, "delay" => "-8s",
    ],
    [
        "title" => "Ficciones", "author" => "Jorge Luis Borges", "genre" => "Short Stories", "year" => "2023",
        "img" => "https://images.unsplash.com/photo-1603058817990-2b9a9abbce86?w=260&h=360&fit=crop&auto=format",
        "bg" => "#1c1508", "accent" => "#0067b2",
        "pos" => "top:42%; left:7%;", "rotate" => -8, "parallax" => 0.07, "delay" => "-1.5s",
    ],
    [
        "title" => "The Trial", "author" => "Franz Kafka", "genre" => "Classic", "year" => "2024",
        "img" => "https://images.unsplash.com/photo-1625053376622-e462848c453f?w=260&h=360&fit=crop&auto=format",
        "bg" => "#1a1010", "accent" => "#0067b2",
        "pos" => "top:38%; right:7%;", "rotate" => 9, "parallax" => -0.06, "delay" => "-6s",
    ],
];
...
$genres = ["Literary Fiction", "Historical", "Poetry", "Biography", "Science",
           "Philosophy", "Translations", "Essays", "Short Stories", "Nature Writing"];
...
<style>
  :root{
    ...
    --rp-green:   #48961d;   /* primary text */
    --rp-blue-d:  #003098;   /* primary color  */
    --rp-blue-l:  #0067b2;   /* light blue      */
    --rp-purple:  #731c84;   /* accent          */
  ...
  *{box-sizing:border-box;}
  html,body{margin:0;padding:0;background:var(--rp-background);color:var(--rp-foreground);
    font-family:'Figtree',sans-serif;overflow-x:hidden;}
  ::-webkit-scrollbar{display:none;} *{scrollbar-width:none;}
  a{color:inherit;text-decoration:none;}
  .rp-text-muted-foreground{color:var(--rp-muted-foreground);}
  ...
  @media(min-width:768px){nav.rp-top-nav{padding:20px 56px;}}
  ...
  .rp-brand-badge span{font-size:9px; font-weight:700; color:#fff; letter-spacing:-0.02em;}
  .rp-brand-name{
    font-family:'Bodoni Moda', serif; font-weight:600; font-size:0.875rem;
    letter-spacing:0.18em; text-transform:uppercase; color:var(--rp-foreground);
  }
  .rp-nav-links{display:none; align-items:center; gap:32px;}
  @media(min-width:768px){.rp-nav-links{display:flex;}}
  .rp-nav-links a{font-size:0.75rem; letter-spacing:0.12em; text-transform:uppercase; color:var(--rp-muted-foreground); transition:color .2s;}
  .rp-nav-links a:hover{color:var(--rp-foreground);}
  .rp-submit-btn{
  ...
  @media(min-width:768px){.rp-submit-btn{display:flex;}}
  ...
  .rp-orb-layer{position:absolute; inset:0; z-index:0; pointer-events:none; overflow:hidden;}
  .rp-orb1{position:absolute; top:-30%; left:-20%; width:70vw; height:70vw; border-radius:50%;
    background:radial-gradient(circle, rgba(72, 150, 29, 0.25) 0%, transparent 65%);
    animation:orb1 20s ease-in-out infinite alternate;}
  .rp-orb2{position:absolute; bottom:-30%; right:-20%; width:75vw; height:75vw; border-radius:50%;
    background:radial-gradient(circle, rgba(0, 48, 152, 0.2) 0%, transparent 65%);
    animation:orb2 25s ease-in-out infinite alternate;}
  .rp-orb3{position:absolute; top:20%; right:5%; width:40vw; height:40vw; border-radius:50%;
  ...
  @media(min-width:1024px){.rp-card{display:block;}}
  .rp-card.rp-entered{opacity:1;}
  .rp-card-inner{
    position:relative; width:108px; height:151px; border-radius:2px; overflow:hidden;
    transition:transform 0.35s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.35s;
    box-shadow:0 16px 48px rgba(0,0,0,0.75), inset -3px 0 10px rgba(0,0,0,0.5);
  }
  .rp-card-spine{position:absolute; left:0; top:0; bottom:0; width:10px; z-index:10;}
  .rp-card-img{position:absolute; inset:0; width:100%; height:100%; object-fit:cover; mix-blend-mode:overlay; opacity:0.75;}
  .rp-card-shade{position:absolute; inset:0;}
  .rp-card-text{position:absolute; inset:0; display:flex; flex-direction:column; justify-content:space-between; padding:10px; z-index:5;}
  .rp-card-text-top{display:flex; align-items:center; justify-content:space-between;}
  .rp-card-genre{font-size:7px; letter-spacing:0.15em; text-transform:uppercase;}
  .rp-card-year{font-size:7px; color:rgba(255,255,255,0.4);}
  .rp-card-title{color:#fff; font-size:8.5px; line-height:1.2; margin:0 0 2px 0; font-weight:600; font-family:'Bodoni Moda', serif;}
  .rp-card-author{color:rgba(255,255,255,0.45); font-size:7px; letter-spacing:0.03em; margin:0;}
  .rp-card-edge{position:absolute; inset:0; border-radius:2px;}

  /* ── Center content ── */
  .rp-center{position:relative; z-index:20; display:flex; flex-direction:column; align-items:center; text-align:center; padding:0 16px; width:100%;}

  .rp-top-label{display:flex; align-items:center; gap:12px; margin-bottom:24px; opacity:0; transform:translateY(16px); transition:opacity .9s .2s, transform .9s .2s;}
  .rp-top-label.rp-entered{opacity:1; transform:translateY(0);}
  .rp-top-label .rp-line-l{height:1px; width:40px; background:linear-gradient(to right, transparent, rgba(240,238,234,0.25));}
  .rp-top-label .rp-line-r{height:1px; width:40px; background:linear-gradient(to left, transparent, rgba(240,238,234,0.25));}
  .rp-top-label span.rp-txt{font-size:10px; letter-spacing:0.35em; text-transform:uppercase; color: #48961d;}

  /* RAINBOW letters */
  .rp-rainbow-row{
    position:relative; display:flex; align-items:flex-end; justify-content:center;
    line-height:0.88; user-select:none; margin-bottom:4px;
    opacity:0; transform:scale(0.94);
    transition:opacity 1.1s .35s cubic-bezier(0.16,1,0.3,1), transform 1.1s .35s cubic-bezier(0.16,1,0.3,1);
  }
  
  .rp-rainbow-row.rp-entered{opacity:1; transform:scale(1);}
  .rp-rletter{
  ...
  .rp-rletter.rp-hovered{filter:brightness(1.4) saturate(1.3);}
  ...
  .rp-publications-label.rp-entered{opacity:1; transform:translateY(0);}

  .rp-sub{opacity:0; transform:translateY(20px); transition:opacity .9s .8s, transform .9s .8s; will-change:transform;}
  .rp-sub.rp-entered{opacity:1; transform:translateY(0);}

  .rp-tagline{
    color:var(--rp-muted-foreground); margin:0 auto 28px auto; max-width:28rem; line-height:1.65;
    font-size:clamp(0.75rem, 1.3vw, 0.9rem); letter-spacing:0.04em;
  }

  .rp-cta-row{display:flex; align-items:center; justify-content:center; gap:16px; margin-bottom:40px;}
  .rp-btn-primary{
    display:flex; align-items:center; gap:10px;
    padding:14px 32px; font-size:0.75rem; letter-spacing:0.15em; text-transform:uppercase;
    border-radius:8px; border:none; color:#fff; cursor:pointer; position:relative; overflow:hidden;
  ...
  .rp-btn-primary:hover{box-shadow:0 0 50px rgba(0, 103, 178, 0.65), 0 8px 32px rgba(0,0,0,0.4); transform:translateY(-2px);}
  .rp-btn-primary svg{transition:transform .2s;}
  .rp-btn-primary:hover svg{transform:translateX(2px);}
  .rp-btn-secondary{
    padding:14px 32px; font-size:0.75rem; letter-spacing:0.15em; text-transform:uppercase;
    border-radius:8px; border:1px solid rgba(72, 150, 29, 0.5); color:#48961d;
    background:transparent; transition:all .3s; cursor:pointer;
  }
  .rp-btn-secondary:hover{border-color:#48961d; color:#48961d; box-shadow:0 0 25px rgba(72, 150, 29, 0.3); transform:translateY(-2px);}

  .rp-stats-row{
    display:flex; align-items:center; justify-content:center; gap:24px;
    border-top:1px solid rgba(240,238,234,0.06); padding-top:clamp(1rem, 2vw, 1.5rem);
  }
  @media(min-width:768px){.rp-stats-row{gap:40px;}}
...
.rp-stat-value {
  ...
  .rp-stat-label{font-size:11px; letter-spacing:0.25em; text-transform:uppercase; color:#3a3942; margin-top:2px;}

  /* ── Scroll indicator ── */
  .rp-scroll-indicator{
    position:absolute; bottom:28px; left:50%; z-index:20;
    display:flex; flex-direction:column; align-items:center; gap:8px;
    transform:translateX(-50%); opacity:0; transition:opacity 1s 1.6s;
  }
  .rp-scroll-indicator.rp-entered{opacity:0.6;}
  .rp-scroll-label{font-size:10px; font-weight:700; letter-spacing:0.3em; text-transform:uppercase; color:#48961d;}
  .rp-scroll-bar{width:1px; height:28px; background:linear-gradient(to bottom, #48961d, transparent); animation:scrollPulse 2s ease-in-out infinite;}

  /* ── Marquee strip ── */
  .rp-marquee-wrap{
    position:absolute; bottom:0; left:0; right:0; z-index:10; overflow:hidden;
    border-top:1px solid rgba(240,238,234,0.05);
    opacity:0; transition:opacity 1s 1.8s;
  }
  .rp-marquee-wrap.rp-entered{opacity:1;}
  .rp-marquee-track{display:flex; white-space:nowrap; padding:10px 0; animation:marquee 30s linear infinite; width:max-content;}
  .rp-marquee-track span.rp-item{margin:0 20px; font-size:10px; letter-spacing:0.25em; text-transform:uppercase; color:var(--rp-muted-foreground);}
  .rp-marquee-track span.rp-item .rp-dot{margin-left:20px; color:var(--rp-border); opacity:0.4;}

  @keyframes orb1{0%{transform:translate(0,0) scale(1);}100%{transform:translate(8%,10%) scale(1.15);}}
  @keyframes orb2{0%{transform:translate(0,0) scale(1);}100%{transform:translate(-10%,-8%) scale(1.12);}}
  @keyframes orb3{0%{transform:translate(0,0) scale(1);}100%{transform:translate(6%,12%) scale(1.1);}}
  @keyframes float-bob{0%,100%{margin-top:0px;}50%{margin-top:-12px;}}
  ...
  #rp-3d-canvas.rp-loaded{opacity:1;}
</style>
...
<section class="rp-hero" id="rp-hero">
  ...
  <div class="rp-center">
    ...
    <div class="rp-sub" id="rp-sub">
      <p class="rp-tagline">Stories that illuminate the human experience —<br>
      from debut voices to Nobel laureates.</p>
      ...
      <div class="rp-cta-row">
        ...
        <button class="rp-btn-secondary">Our Authors</button>
      </div>
...
(() => {
  ...
  const cursorSmooth = { x: mouse.x, y: mouse.y };
  const ringSmooth = { x: mouse.x, y: mouse.y };
  ...
  if (typeof gsap !== 'undefined') {
    ...
    tl.to('#rp-topLabel',    { opacity: 1, y: 0, duration: 0.8 }, 0.2)
      .to('#rp-rainbowRow',  { opacity: 1, scale: 1, duration: 1.1, ease: 'expo.out' }, 0.35)
      .to('#rp-pubLabel',    { opacity: 1, y: 0, duration: 0.7 }, 0.55)
      .to('#rp-sub',         { opacity: 1, y: 0, duration: 0.7 }, 0.7)
      .to('#rp-scrollIndicator', { opacity: 0.6, duration: 0.6 }, 1.4)
      .to('#rp-marqueeWrap', { opacity: 1, duration: 0.6 }, 1.5)
      .to('.rp-card',        { opacity: 1, duration: 0.8, stagger: { each: 0.12, from: 'random' } }, 0.4);
    ...
    if (primaryBtn) {
      primaryBtn.addEventListener('mousemove', function (e) {
        ...
        gsap.to(primaryBtn, { x: x * 0.18, y: y * 0.18, duration: 0.4, ease: 'power3.out' });
      });
      primaryBtn.addEventListener('mouseleave', function () {
        gsap.to(primaryBtn, { x: 0, y: 0, duration: 0.6, ease: 'elastic.out(1, 0.4)' });
      });
    }
  ...
  letterEls.forEach((el, i) => {
    el.addEventListener('mouseenter', () => { hoveredLetter = i; el.classList.add('rp-hovered'); });
    el.addEventListener('mouseleave', () => { hoveredLetter = null; el.classList.remove('rp-hovered'); });
  });
...
(function () {
    ...
    var accentDefs = [
        { geo: new THREE.OctahedronGeometry(0.22, 0), pos: [3.8, 1.2, -2.5], color: 0x0067b2 },
        { geo: new THREE.TetrahedronGeometry(0.18, 0), pos: [-4.5, -1.5, -3], color: 0x731c84 },
        { geo: new THREE.TorusGeometry(0.25, 0.07, 6, 12), pos: [4.2, -2.8, -2], color: 0x48961d },
        { geo: new THREE.TorusKnotGeometry(0.16, 0.04, 24, 6), pos: [-3.8, 2.5, -2.8], color: 0x003098 },
        { geo: new THREE.OctahedronGeometry(0.15, 0), pos: [0, 3, -2], color: 0x48961d },
        { geo: new THREE.DodecahedronGeometry(0.2, 0), pos: [-5, 0.5, -3.5], color: 0x0067b2 },
    ];
    accentDefs.forEach(function (d) {
        ...
        m.userData = {
            ...
            rot: { x: (Math.random() - 0.5) * 0.03, y: (Math.random() - 0.5) * 0.03, z: (Math.random() - 0.5) * 0.02 },
        };
        accentGroup.add(m);
    });
    ...
    heroEl.addEventListener('mouseenter', function() { mouseActive = true; });
    heroEl.addEventListener('mouseleave', function() { mouseActive = false; });
    ...
    /* fade canvas in */
    setTimeout(function () { canvas.classList.add('rp-loaded'); }, 300);
})();
</script>
...
(function () {
    ...
    function animateCounters(root) {
        (root || document).querySelectorAll('.counter-num').forEach(function (el) {
            ...
            gsap.to(obj, {
                ...
                duration: 1.8,
                ease: 'power2.out',
                onUpdate: function () { el.textContent = Math.floor(obj.val).toLocaleString() + suffix; }
            });
        });
    }
    ...
    if (reduceMotion) {
        ...
        heroTl
            .from('.hero-content .section-label', { opacity: 0, y: 18, duration: 0.6 })
            .from('.hero-content h1', { opacity: 0, y: 28, duration: 0.8 }, '-=0.35')
            .from('.hero-content p', { opacity: 0, y: 18, duration: 0.7 }, '-=0.5')
            .from('.hero-buttons a', { opacity: 0, y: 18, duration: 0.6, stagger: 0.15 }, '-=0.4')
            .from('.hero-stats > div', { opacity: 0, y: 22, duration: 0.6, stagger: 0.1 }, '-=0.3')
            .add(function () { animateCounters(document); }, '-=0.2');
    }
    ...
    document.querySelectorAll(tiltSelectors).forEach(function (el) {
        ...
        var qx = gsap.quickTo(el, 'rotationY', { duration: 0.5, ease: 'power3.out' });
        var qy = gsap.quickTo(el, 'rotationX', { duration: 0.5, ease: 'power3.out' });
        el.addEventListener('mousemove', function (e) {
            var r = el.getBoundingClientRect();
